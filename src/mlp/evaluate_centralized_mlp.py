...
- Evaluate the centralized MLP model and generate charts.
- Run main_MLP.py first.

Usage:
    python3 evaluate_centralized_mlp.py -cls mc
    python3 evaluate_centralized_mlp.py -cls b
"""
...
os.makedirs('charts/mlp', exist_ok=True)
...
if __name__ == '__main__':
    ...
    if not os.path.exists(model_path):
        print(f'ERROR: {model_path} not found. Run main_MLP.py -cls {cls} first.')
        sys.exit(1)
    ...
    if not os.path.exists(history_path):
        print(f'ERROR: {history_path} not found. Run main_MLP.py -cls {cls} first.')
        sys.exit(1)
    ...
    plt.suptitle(f'Centralized MLP {suffix.capitalize()} - Training Curves')
    plt.tight_layout()
    plt.savefig(f'charts/mlp/mlp_{suffix}_training_curves.png')
    plt.show()
    ...
    plt.title(f'Centralized MLP {suffix.capitalize()} - Confusion Matrix')
    plt.xlabel('Predicted')
    plt.ylabel('Actual')
    plt.tight_layout()
    plt.savefig(f'charts/mlp/mlp_{suffix}_confusion_matrix.png')
    plt.show()
    ...
    plt.ylim(0, 1.15)
    plt.title(f'Centralized MLP {suffix.capitalize()} - Per-Class Metrics')
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'charts/mlp/mlp_{suffix}_per_class_metrics.png')
    plt.show()
    ...
    roc_auc = None
    if cls == 'b':
        fpr_curve, tpr_curve, _ = roc_curve(y_test, y_pred_probs[:, 1])
        roc_auc = auc(fpr_curve, tpr_curve)
        plt.figure(figsize=(6, 5))
        plt.plot(fpr_curve, tpr_curve, label=f'AUC = {roc_auc:.3f}')
        plt.plot([0, 1], [0, 1], '--', color='gray')
        plt.title('Centralized MLP Binary - ROC Curve')
        plt.xlabel('False Positive Rate')
        plt.ylabel('True Positive Rate')
        plt.legend()
        plt.tight_layout()
        plt.savefig('charts/mlp/mlp_binary_roc_curve.png')
        plt.show()
    ...
    pd.DataFrame([metrics]).to_csv(f'charts/mlp/mlp_{suffix}_metrics.csv', index=False)
    with open(f'charts/mlp/mlp_{suffix}_report.json', 'w') as f:
        json.dump(report, f, indent=4)

    print(f'\nAll charts and metrics saved to charts/mlp/')
    print('Done!')